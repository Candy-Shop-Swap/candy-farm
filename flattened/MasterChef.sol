...
// File: contracts\CandyToken.sol

pragma solidity 0.6.12;


// CandyToken with Governance.
contract CandyToken is BEP20('Candies', 'Candy') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CANDY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CANDY::delegateBySig: invalid nonce");
        require(now <= expiry, "CANDY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CANDY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CANDYs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CANDY::_writeCheckpoint: block number exceeds 32 bits");
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    function safeCandyTransfer(address _to, uint256 _amount) public onlyOwner {
...
// MasterChef is the master of Candy. He can make Candy and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CANDY is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCandyPerShare; // Accumulated CANDYs per share, times 1e12. See below.
    }
    ...
    // Update the given pool's CANDY allocation point. Can only be called by the owner.
    ...
    function pendingCandy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCandyPerShare = accCandyPerShare.add(candyReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accCandyPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 candyReward = multiplier.mul(candyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        candy.mint(devaddr, candyReward.div(10));
        candy.mint(address(syrup), candyReward);
        pool.accCandyPerShare = pool.accCandyPerShare.add(candyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit CANDY by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCandyPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw CANDY by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake CANDY tokens to MasterChef
    ...
    // Withdraw CANDY tokens from STAKING.
    ...
    function safeCandyTransfer(address _to, uint256 _amount) internal {
        syrup.safeCandyTransfer(_to, _amount);
    }
