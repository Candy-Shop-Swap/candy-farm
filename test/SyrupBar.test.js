...
contract('SyrupBar', ([alice, bob, carol, dev, minter]) => {
  ...
  it('safeCandyTransfer', async () => {
    assert.equal(
      (await this.candy.balanceOf(this.syrup.address)).toString(),
      '0'
    );
    await this.candy.mint(this.syrup.address, 1000, { from: minter });
    await this.syrup.safeCandyTransfer(bob, 200, { from: minter });
    assert.equal((await this.candy.balanceOf(bob)).toString(), '200');
    assert.equal(
      (await this.candy.balanceOf(this.syrup.address)).toString(),
      '800'
    );
    await this.syrup.safeCandyTransfer(bob, 2000, { from: minter });
    assert.equal((await this.candy.balanceOf(bob)).toString(), '1000');
  });
});
